...
export const Egresos = ({ lista, EliminarE, ActualizarE }) => {
  ...
  //Para referenciar se usan los mismo Nombres que se encuentran descritos en el modelo de la API (API_Prisma)
  return (
    <div className={styles.tarjeta}>
      <h3>Egresos</h3>
      {lista.length === 0 ? (
        <p>
          No hay <strong>egresos</strong> registrados.
        </p>
      ) : (
            ...
            {lista.map((egreso, index) => (
              <tr key={index}>
                {modoEditar === index ? (
                  ...
                  <>
                    {/*Siguiendo con lo de arriva, egreso.Nombre y asi sucesivamente con cada parametro de la entidad
